...
if (isset($_POST['add_attendance'])) {
    ...
    // Increase user rating by 2
    $db->prepare("UPDATE users SET rating = COALESCE(rating,800) + 2 WHERE id = ?")->execute([$user_id]);
...
<head>
    <meta charset="UTF-8">
    <title>Admin Branch Attendance - NOVEL NEST</title>
...
<body>
    ...
    <div class="admin-box">
        <h2>Branch Attendance Management</h2>
        ...
        <form method="get" style="margin-bottom:16px;display:flex;gap:8px;align-items:center;">
            ...
            <?php if ($search_id_name || $search_branch || $search_date): ?>
                <a href="admin_branches.php" style="color:#888;">Clear</a>
            <?php endif; ?>
        </form>
        <form method="post">
        <table>
            ...
            <tr>
                ...
                <td class="actions">
                    <?php if (isset($_GET['edit']) && $_GET['edit'] == $e['id']): ?>
                        <!-- Inline edit form with dropdowns -->
                        <form method="post" style="display:inline;">
                            <input type="hidden" name="edit_id" value="<?= $e['id'] ?>">
                            <select name="edit_user_id" required>
                                <?php foreach ($user_list as $u): ?>
                                    <option value="<?= $u['id'] ?>" <?= $e['user_id'] == $u['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($u['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <select name="edit_branch_id" required>
                                <?php foreach ($branch_list as $br): ?>
                                    <option value="<?= $br['id'] ?>" <?= $e['branch_id'] == $br['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($br['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="time" name="edit_entry_time" value="<?= $e['entry_time'] ?>" required>
                            <input type="time" name="edit_leave_time" value="<?= $e['leave_time'] ?>">
                            <button type="submit" class="action-btn edit-btn">Save</button>
                            <a href="admin_branches.php" class="action-btn">Cancel</a>
                        </form>
                    <?php else: ?>
                        <a href="admin_branches.php?edit=<?= $e['id'] ?>" class="action-btn edit-btn">Edit</a>
                        <a href="admin_branches.php?delete=<?= $e['id'] ?>" class="action-btn delete-btn" onclick="return confirm('Delete this attendance?')">Delete</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
